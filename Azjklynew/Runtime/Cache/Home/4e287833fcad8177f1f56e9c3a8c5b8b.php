...
<head>
	...
	<title>注册WebInsights_WebInsights官网</title>
		...
		<div class="mainbox">
		    ...
		    <div class="sign_box">
		    	...
		    	<div class="signmain">
					...
					<div class="input_item">
						<p class="errortip">请输入验证码</p>
						<input type="text" name="yzm" placeholder="验证码"/><button class="yzm_btn" onclick="sendCode(this)">获取验证码</button>	
					</div>
					<div class="input_item">
						<p class="errortip">请输入密码</p>
						<input type="password" name="password" placeholder="输入密码"/>
					</div>
					<div class="input_item">
						<p class="errortip">您输入的密码不一致</p>
						<input type="password" name="password2" placeholder="确认密码"/>
					</div>
<!--
					<div class="input_item">
						<p class="errortip"></p>
						<input type="text" name="com_name" placeholder="公司名称"/>
					</div>
-->
					<div class="input_item">
						<button name="sgin_btn">注册</button>
					</div>
		...
		<div class="item_bottom">
			<p>已有账号？<a href="/index.php?m=home&c=member&a=logins">立即登录</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="/">返回首页</a></p>
		</div>
	</div>
	</div>
	...
	<script>
		$(document).ready(function () {
			...
			$("button[name='sgin_btn']").click(function(){
				...
				var passwordselecter2 = $("input[name='password2']");
				var password_val2 = passwordselecter2.val();
				var yzmselecter = $("input[name='yzm']");
				var yzm_val = yzmselecter.val();
				var com_nameselecter = $("input[name='com_name']");
				var com_nameselecter_val = com_nameselecter.val();
				...
				yanz_pass(password_val2,passwordselecter2);//执行再次输入密码验证
				yanz_pass2(password_val,passwordselecter,password_val2,passwordselecter2)//两次密码一致性验证
				yanz_yzm(yzm_val,yzmselecter);//执行验证码验证
				if(flag == 1 && flag_phone == 1 && flag_password == 1 && flag_yzm == 1 && flag_password_compare ==1){
					//postlist("/Azjklynew/Home/Controller/reg.php", pagedata, _this);
